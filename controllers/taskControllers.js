...
exports.postTask = async (req, res) => {
  try {
    ...
    const task = await Task.create({ user: req.user.id, description,status:"Not Complited" });
...
exports.putStatusTask = async (req, res) => {
  try {
    const status  = "Complited";
    console.log(status)
    if (!status) {
      return res.status(400).json({ status: false, msg: "Description of task not found" });
    }
    ...
    task = await Task.findByIdAndUpdate(req.params.taskId, { status }, { new: true });
    res.status(200).json({ task, status: true, msg: "Task updated successfully.." });
  }
  catch (err) {
    console.error(err);
